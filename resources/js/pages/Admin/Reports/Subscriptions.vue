...
<template>
    ...
    <Button @click="router.visit(route('admin.reports.subscriptions'))" variant="outline" size="icon" title="Limpiar Filtros">
        <FilterX class="h-4 w-4" />
    </Button>

    <a 
        :href="route('admin.reports.subscriptions.export', filters)"
    ...
    >
        Exportar a Excel
            ...
            <div v-if="subscriptions.data.length === 0" class="flex items-center justify-center h-[40vh] text-muted-foreground">
            ...
            <div v-else class="overflow-x-auto">
                ...
                <div v-if="subscriptions.links.length > 3" class="flex justify-center mt-6">
                    <nav class="flex space-x-1">
                        <Link v-for="(link, index) in subscriptions.links" 
                           :key="index"
                           :href="link.url ?? '#'"
